...
// hooks
  ...
  useEffect(() => {
    ...
    function pagRight(){
      if(sliderWdt.current.scrollLeft == sliderWdt.current.scrollWidth || sliderWdt.current.scrollLeft + 210 > sliderWdt.current.scrollWidth){
    ...
    function pagLeft(){
      if(sliderWdt.current.scrollLeft == 0 || sliderWdt.current.scrollLeft - 210 < 0){
        sliderWdt.current.scrollTo({
...
// component
    const cards = ongoingHome.map((i) => {
      return (<OngItem imgage={i.images.webp.image_url} descript={i.title} rate={i.score}/>);
    })

  return (
    <div className='ongoingSliderWrapper'>
      <button ref={perpag} className='pervous pag' onClick={pagLeft}>{"<"}</button>
      <div ref={sliderWdt} className='Ongoing'>
        {cards}
      </div>  
      <button nextpag={nextpag} className='next pag' onClick={pagRight}>{">"}</button>
    </div>
